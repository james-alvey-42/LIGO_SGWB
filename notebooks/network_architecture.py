...
class POPExpLoss(tf.keras.losses.Loss):
    ...
    def call(self, model_label, model_pred):
        model_pred = leaky_parity_odd_power(model_pred, alpha=2)
...
#         dense_model.summary()
        ...
        if self.decay_rate is not None:
            lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
                ...
                staircase=True  # Ensures discrete decay steps
            )
            optimizer = tf.keras.optimizers.Adam(learning_rate=lr_schedule)
            dense_model.compile(optimizer=optimizer, loss=ExpLoss())  # Removed `metrics=[lr_metric]`
        else:
            optimizer = tf.keras.optimizers.Adam(learning_rate=self.learning_rate)
            dense_model.compile(optimizer=optimizer, loss=ExpLoss())
